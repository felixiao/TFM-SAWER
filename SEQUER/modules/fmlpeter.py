...
from modules.base import PositionalEncoding, TransformerEncoderLayer, TransformerEncoder, generate_sequer_mask,generate_fmlpeter_mask, generate_fmlpeter_mask_new, MLPETER,MLP
...
class FMLPETER(BaseModel):
    ...
    def predict_context(self, hidden):
        context_prob = self.hidden2token(hidden[self.hist_len])  # (batch_size, n_token)
    ...
    def predict_rating(self, hidden):
        ...
        rating = self.recommender(hidden[0])  # (batch_size, seq_len)
    ...
    def forward(self, user, item, text, **kwargs):
        ...
        i_src = self.item_embeddings(item).permute(1,0,2)  # (max_seq_len, batch_size, emsize)
        # i_src = self.item_embeddings(item[:,-1:]).permute(1,0,2)  # (1, batch_size, emsize)
        # print(f'ITEM len:{item.shape}, item:{item[0,-1]}')
        # print(f'FMLP Device: {self.fmlp_model.device}')
        
        # u_src = self.fmlp_model(item).permute(1,0,2) # (batch_size, max_seq_len, emsize)       ---> (max_seq_len,batch_size,emsize) 
        # if self.multi_gpu:
