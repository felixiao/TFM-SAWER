...
class SAWER(BaseModel):
    ...
    def predict_context(self, hidden):
        context_prob = self.hidden2token(hidden[self.hist_len-1])  # @ Last Item (batch_size, n_token)
    ...
    def predict_rating(self, hidden):
        ...
        rating = self.recommender(hidden[self.hist_len])  # @User (batch_size, seq_len)
