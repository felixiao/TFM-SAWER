...
from . import PETER, SEQUER, Att2Seq, NRT, ContinuousPromptLearning
...
class Trainer:
    def __init__(self, dataset, fold, prediction_path, model_name, model_suffix, cfg, device, test_flg=False,
                 gen_flg=False, check_convergence=False):
        ...
        self.device = device
        self.load_data(dataset, fold, self.seq_mode, model_name, test_flg)
        self.model_path = os.path.join(CKPT_PATH, f'{model_name}{model_suffix}_{dataset}_{fold}.pth')
        ...
        self.exp_metadata = {'model_name': model_name + model_suffix, 'dataset': dataset, 'split_ix': fold, 'config': cfg_txt,
    ...
    def load_data(self, dataset, fold, seq_mode, model_name, test_flg=False):
        print(now_time() + f'Loading data {os.path.join(DATA_PATHS[dataset], str(fold))}')
        self.tokenizer = None
        if 'pepler' in model_name.lower():
            self.tokenizer = AutoTokenizer.from_pretrained('gpt2', bos_token=BOS_TOK, eos_token=EOS_TOK, pad_token=PAD_TOK)
    ...
    def build_model(self, model_name, cfg, device):
        ...
        self.model = None
        if 'peter' in model_name.lower():
            self.model = PETER(self.src_len, self.tgt_len, self.word2idx[PAD_TOK],
                               self.nuser, self.nitem, self.ntokens, cfg).to(device)
        elif 'sequer' in model_name.lower():
            self.model = SEQUER(self.src_len, self.tgt_len, self.word2idx[PAD_TOK],
                                self.nuser, self.nitem, self.ntokens, cfg).to(device)
        elif 'att2seq' in model_name.lower():
            self.model = Att2Seq(self.word2idx[PAD_TOK], self.nuser, self.nitem, self.ntokens, cfg).to(device)
        elif 'nrt' in model_name.lower():
            self.model = NRT(self.word2idx[PAD_TOK], self.nuser, self.nitem, self.ntokens, cfg).to(device)
        elif 'pepler' in model_name.lower():
            self.model = ContinuousPromptLearning.from_pretrained('gpt2', self.nuser, self.nitem)
            self.model.resize_token_embeddings(self.ntokens)
    ...
    def train_epoch(self):
        ...
        total_sample = 0
        while True:
            if (self.check_convergence and self.train_data.step == 0) or not self.check_convergence:
    ...
    def get_start_ixs(self, batch):
        start_ixs = torch.zeros((batch.shape[0], 1), dtype=torch.int)
    ...
    def compute_metrics(self, data, rating_predict, idss_predict, context_predict):
        ...
        print(now_time() + 'FMR {:7.4f}'.format(results['FMR']))
