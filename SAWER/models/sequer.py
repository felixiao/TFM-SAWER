...
from .base import PositionalEncoding, TransformerEncoderLayer, TransformerEncoder, generate_sequer_mask
from .peter import BaseModel
...
class SEQUER(BaseModel):
    def __init__(self, src_len, tgt_len, pad_idx, nuser, nitem, ntoken, cfg):
        ...
        self.user_last = cfg.get('user_last', False)
        ...
        self.attn_mask = generate_sequer_mask(src_len, tgt_len, self.user_last, self.bert_like)
        self.init_weights()
    ...
    def forward(self, user, item, text, **kwargs):
        ...
        if kwargs.get('filter_last', False):
            rating = rating[:, -1]
