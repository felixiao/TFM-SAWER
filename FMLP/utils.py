...
sequential_data_list = ['Beauty','Sports_and_Outdoors','Toys_and_Games','Yelp','Movie_and_TV_GT5','Movie_and_TV_index','reviews_new']
...
def get_user_seqs_and_sample_from_pickle(data_file):
    # peter_data = pd.read_csv(data_file,dtype={'user':object,'item':object,
    ...
    peter_data['len_hist_item'] = peter_data.apply(lambda row:len(row['hist_item_index']),axis=1)
    peter_data = peter_data[peter_data['len_hist_item'] > 2]
    ...
    return list(user_seq), max_item, num_users, list(sample_seq),list(neg_item)
...
def get_dataloder(args,seq_dic):
    ...
    if args.data_name in sequential_data_list:
        train_dataset = FMLPRecDataset(args, seq_dic['user_seq'],seq_dic['neg_item'], data_type='train')
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq'], seq_dic['neg_item'],test_neg_items=seq_dic['sample_seq'], data_type='valid')
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

        test_dataset = FMLPRecDataset(args, seq_dic['user_seq'], seq_dic['neg_item'],test_neg_items=seq_dic['sample_seq'], data_type='test')
